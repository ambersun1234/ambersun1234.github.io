...
date: 2023-12-31
description: 本文將會藉由導讀 JMS, MQTT 以及 AMQP 的標準，理解 Apache Kafka 以及其在維運相關會遇到的問題
categories: [database]
tags: [distributed, cluster, message queue, publisher, consumer, producer, subscriber, event, kafka, amqp, mqtt, jms, dlq, pubsub, p2p, pull protocol, push protocol, consumer group, rebalancing, static membership]
math: true
---
...
### Partition
為了能夠突破單個磁碟所能提供的吞吐量限制\
Kafka 將 topic 切分成若干個 partition，每個分區都是獨立的日誌

publisher 不需要經過任何 routing 系統，它可以直接將資料寫到 partition 的 leader 上\
至於說他要怎麼知道送到哪，所有的 Kafka node 都有辦法直接回覆說你要去哪裡等等的問題

不過，就算我可以直接跟節點溝通，它並不會幫你決定資料要寫到哪個 partition 上\
在某種程度上你需要自己決定位置\
通常有以下幾種方法
1. 使用隨機的 load balancing 策略將資料隨機分散到不同 partition 上
2. 或者是你使用 hash-based 的方式將資料分散到特定 partition 上(相同的 `key` 會永遠出現在相同的 partition 上)

又因為 Kafka 的資料是基於 log-like 的結構\
寫入的資料都是有順序性的，不過 **僅限於 partition 內部**\
這可以保證，consumer 在讀取的時候，一定是按照順序的(同一時間一個 partition 只能有一個 consumer 在讀取，所以如果當前 partition 處理被卡住，後面的資料都會被卡住)\
這意味著，consumer 的數量如果超過 partition 數量，多的 consumer 會閒置\
這種 ***粗顆粒度*** 的作法，對於 Kafka 來說是一種取捨，好處是處理速度可以很快，壞處則是對於平行處理或訊息排序不重要的場合不那麼適用

> 不過當資料量大的時候，Kafka 會做 `re-balancing`
...
比如說 **RabbitMQ** 會透過 acknowledgement 來確保資料被正確的讀取以及處理\
所以可以安全的刪除，但這會有問題
1. consume 了，處理了但是失敗了，同一個資料會被重複處理
2. 一筆資料現在必須包含 ack 的欄位，用以紀錄是否已經被正確的處理

> 有關 RabbitMQ 可以參考 [資料庫 - 解耦助手 RabbitMQ \| Shawn Hsu](../../database/database-rabbitmq)
...
我們實際上在操作 Kafka 的時候，是透過 `Consumer Group` 的\
機器可能會因為各種原因下線，當你的 Consumer 掛掉之後，必須有另一個人來接手(可能是立馬 provision 的，正在執行中的又或者是閒置的 Consumer)\
這種過程稱為 `Rebalancing`

但不一定是掛掉才會觸發 Rebalancing，你新增了 Consumer 或是調整了 Partition 的數量都會觸發 Rebalancing\
為了避免每次調整，所有人都必須重新協調，很明顯有些人根本不會被動到，在 *Static Membership* 的機制下，每個 Consumer 都有一個自己的 id，只要 id 沒有變，它就不會被 rebalance

> 你可以參考 [Producer and Consumer](#producer-and-consumer)\
...
> 可參考 [Partition](#partition) 以及 [Consumer Rebalancing Protocol](#consumer-rebalancing-protocol)
...
一種作法是你先把舊的讀完再讀新的，因為分區內是保證順序的\
但是你在 code 裡面就要判斷讀完了沒，怎麼切過去等問題

又或者是開一個新的 `topic`\
一樣分兩邊 `topic`, 新的資料去 `topic_new`\
其實作法跟上面的 `P1`, `P2` 一樣，只是一個在原本的 topic 上，一個是新的 topic 上

## Ordering of Messages
我們一直提到說，partition 內部的資料是有序的\
partition 之間是無序的

那你要怎麼保證你讀的順序一定是正確的？\
其中顯而易見的就是一個 partition 給一個 consumer 讀\
並且啟用 `Idempotent Producer`(現在是預設開啟的)\
它可以偵測重複的資料，並同時保證順序的正確性

不過單一 consumer 的情況下，如果 producer 的資料量很大你會來不及處理，一個方法就是 [New Partition Assignment](#new-partition-assignment) 的作法\
多線程？ Maybe? 如果你不在意順序的話\
並且開 thread pool 下去處理(i.e. 一個主要的 thread 負責讀取，其他的 thread 負責處理)\
對於 offset 以及順序的處理要非常小心\
總的來說是可以做的，看你需求
