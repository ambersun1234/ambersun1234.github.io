...
有哪些東西是不會被 GC 處理的?\
在經過 [Escape Analysis](#escape-analysis) 確定，會留在當前 stack 的資料，就是可預測的\
這些可預測的資料並不需要 GC 定時管理\
他們位於 stack 上，普遍被稱為 `stack allocation`

針對那些動態的記憶體存取，比方說根據使用者的輸入決定記憶體大小\
在經過 [Escape Analysis](#escape-analysis) 確定，屬於 **不可預測的**\
或者是指標類型的，因為你不太能確定這塊記憶體會被怎麼用以及哪時候可以安全的被回收\
這種動態的記憶體存取就是 GC 需要主動介入的

## Escape Analysis
Golang 跟我們以往認識的語言不同，單純的 local variable 也有可能會被放到 heap\
比方說
...
./a.go:3:6: can inline stayOnHeap
./a.go:8:6: can inline stayOnStack
./a.go:13:6: can inline main
./a.go:14:12: inlining call to stayOnHeap
./a.go:15:13: inlining call to stayOnStack
./a.go:4:5: moved to heap: x
./a.go:9:13: new(int) does not escape
./a.go:15:13: new(int) does not escape
```

你可以很清楚的看到，`stayOnHeap` 裡面的變數實際上會 "escape"(i.e. `moved to heap`)\
啥意思呢？ 因為 `x` 離開 function 之後還存活著，也就是說你無法確定他哪時候不用了\
相比 stack 來說，離開 function 之後就可以不用管他了，也不會有人會去存取他\
因此這個行為被稱之為 "escape"\
所以單純的說 `non-pointer` 的變數不會被 GC，在某些語言下是不準確的

"escape" 的資料會跑到 heap\
GC 因此要掃描更多的資料，因為 GC 處理 heap 上的東西\
也就是說，當你的 heap 越大，GC 的開銷可能也會越大，進而導致你的程式變慢
...
![](https://inside.java/resources/heapgc_defrag--E4xZ7xtxg3wqropcpGF3QFfsXTKzZfhZB2DefDBI.png)
...
+ [Week37 -我當時害怕極了，原來Golang用指標是母湯的 [Server的終局之戰系列]](https://ithelp.ithome.com.tw/articles/10254308)
